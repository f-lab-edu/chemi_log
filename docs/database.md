...
ERD 를 다시 설계했습니다. 컬럼 여덟 개가 사라졌고 질문 스냅샷이 값 복사에서
...
**`HEAD` 로 조회하면 같은 경로에 새 스키마가 있어서 명령은 성공합니다.** 그런데 나오는 것은
재설계 전 내용이 아니라 지금 내용입니다. 두 스키마가 같다고 오해하게 됩니다.
...
> **이 절은 임의 수정 금지 대상입니다.**
> `question` 의 `category`, `content`, `option_a`, `option_b` 를 UPDATE 하는 코드가 생기면
> 이미 만들어진 방의 질문이 바뀝니다. 이 전제가 깨지면 스키마 전체가 의미를 잃습니다.
...
**원본이 바뀌지 않으므로** 참조만 해도 그 요구가 지켜집니다.
...
컬럼을 없앤 자리에 **`id` 가 식별자이면서 순서 노릇을 합니다.** 그 사실이 스키마에
...
그래도 컬럼으로 둡니다. `UNIQUE (room_id, nickname_key)` 를 걸어야 합니다.
...
48 은 그 상한에 여유를 둔 값입니다. **36 아래로 줄이면 그 저장이 실패합니다.**
...
`ascii` 로 되돌리면 위 두 가지가 다시 생깁니다. 저장 공간을 아끼려는 것이라면 이득보다 손해가 큽니다.

### submitted_at 은 NOW(6) 로 채웁니다

`created_at` 네 개(`question`, `room`, `participant`, `answer`)는 DB 가
`DEFAULT CURRENT_TIMESTAMP(6)` 로 만듭니다.
`submitted_at` 은 제출 시점에 UPDATE 하는 값이라 DEFAULT 를 쓸 수 없습니다.

**`created_at` 을 엔티티에 매핑할 때는 `@Generated(event = INSERT)` 와
`insertable = false, updatable = false` 를 함께 붙입니다.** 평범한 `@Column` 으로 두면
Hibernate 가 INSERT 에 `NULL` 을 실어 보내고 `1048 Column 'created_at' cannot be null` 로
거부됩니다. `created_at` 을 가진 테이블이 늘면 이 규칙도 따라갑니다.
...
**아래는 빠뜨리지 않았습니다. 앱에 맡겼습니다.** 다시 넣기 전에 이 절을 읽으십시오.
...
**영문 문항은 대소문자만 다른 두 문항을 같은 카테고리에 넣을 수 없습니다.** 넣으면
시드가 `1062` 로 멈추고, 엔트리포인트가 `set -e` 라 **컨테이너가 기동하지 않습니다.**
지금 시드는 한글 120문항이라 걸리는 쌍이 없습니다.

**`content` 의 collation 을 지우고 서버 기본값에 맡기면 막는 값이 달라집니다.** `NO PAD` 는
