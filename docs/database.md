...
**공백을 지우는 것은 키뿐이다.** 표시값에는 내부 공백이 남는다. PRD 7장이 두 규칙을
나눠 뒀다. `지 은` 으로 입력한 사람은 `지 은` 으로 보이고, 중복 판정에서만 `지은` 과 같아진다.
표시값까지 지우면 사용자가 입력한 이름이 화면에서 바뀐다.
...
PRD 7장이 요구한 중복 판정 기준은 대소문자 통합과 공백 제거까지이므로 accent는 구분해야 한다.
